...
class BootRecord:
    ...
    def __init__(self, width=1300, higth=600):
        self.serial_code = ""
        self.bg = "#C7EDCC"
        self.list_index = 0
    ...
    def control_option(self):
        ...
        file_btn.place(x=1110, y=56, anchor=anchor)

        self.label_serial_code.place(x=150, y=120, anchor=anchor)

        self.scrollbar.place(x=590, y=126, height=400, anchor=anchor)
        ...
        self.label_text_console_name.place(x=710, y=120, anchor=anchor)
        self.text_console.config(state=tk.DISABLED)
        self.text_console.place(x=1156, y=124, height=400, anchor=anchor)
    ...
    def get_command(self):
        if CPO.product_mode == "IC218" or CPO.product_mode == "IM218":
            command = "/home/pi/work/openblt/Host/canbootcli -n=1 -d=can0"
        elif CPO.product_mode == "IC216":
            command = "/home/pi/work/openblt/Host/canbootcli -n=1 -d=can0"
        else:
            command = "/home/pi/work/openblt/Host/canbootcli -n=1 -d=can0"

        command += " %s" % self.file_entry_var.get()
        return command
    ...
    def run_command(self):
        ...
        for line in iter(popen.stdout.readline, b''):
            ...
            line = line.replace("\x08", "").replace("\x1b", "")\
                .replace("[0m", "").replace("[32m", "").replace("[33m", "")
